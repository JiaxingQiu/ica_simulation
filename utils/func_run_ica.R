run_ica = function(gen_data_obj, aci_iter = 25){
  ...
  return(list(smat_hat = t(ICA_res$S), 
              ...
              ACI = ACI(gen_data_obj$mixmat, mixmat_hat, iter = aci_iter), # adjusted concordance index, which evaluates how well our estimates of the mixing matrix match up with the original mixing matrix
...
iterate_ica <- function(gen_data_obj, sim_condition){
  ...
  aci_iter <- sim_condition$aci_iter
  ...
  for(j in 1:ica_iter ){
    ica_run <- run_ica(gen_data_obj, aci_iter = aci_iter)
